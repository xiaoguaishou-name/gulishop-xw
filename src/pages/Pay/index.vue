  ...
  <div class="pay-main">
    <div class="pay-container">
      <div class="checkout-tit">
        ...
        <div class="paymark">
          <span class="fl">请您在提交订单<em class="orange time">4小时</em>之内完成支付，超时订单会自动取消。订单号：<em>{{$route.query.orderNum}}</em></span>
...
<script>
  ...
  export default {
    ...
    methods:{
      async getOrderInfo(){
        const result = await this.$API.reqOrderInfo(this.$route.query.orderNum)
        if(result.code === 200){
      ...
      async pay(){
        try {
          ...
          this.$alert(`<img src="${imgUrl}" />`,"请使用微信扫码支付",{
            ...
            showCancelButton:true,   //取消按钮是否显示
            cancelButtonText:"支付中遇到问题" , //取消按钮显示文本内容
            confirmButtonText:"我已成功支付",  //确定按钮显示文本内容
            center:true,  //文本内容居中
            beforeClose:(action,instance,done)=>{
              if(action === 'confirm'){
                if(this.status === 200){
                  ...
                  this.$message.warning('衰仔，不给钱想白嫖吗？')
                }
              }else if(action === 'cancel'){
                clearInterval(this.timer)
                this.timer = null
                this.$message.success('请联系客服小姐姐')
                done()
              }
            }
          }).then(()=>{}).catch(()=>{})
        ...
        if(!this.timer){
          this.timer = setInterval(async ()=>{
            ...
            if(result.code === 200){
              //支付成功的状态码保存一下，为了用户点击成功支付按钮时去判断
              this.status = 200
              clearInterval(this.timer)
              this.timer = null
              this.$router.push('/paysuccess')
              this.$msgbox.close() //手动关闭盒子
            }
          },3000)
        }
      }
